...
const networkReq = { headers: { 'x-forwarded-for': '203.0.113.10, 10.0.0.2' }, socket: { remoteAddress: '10.0.0.3' } }
const networkKey = requestNetworkKey(networkReq)
assert(networkKey.length === 24, 'network rate key must be hashed')
assert(!networkKey.includes('203.0.113.10'), 'raw IP must not be stored in rate key')
...
console.log('Gateway rate limit test OK: fixed window, Retry-After, reset and privacy-safe keys verified')
